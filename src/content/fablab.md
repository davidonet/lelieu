---
title: Le fablab
slug: fablab
color: red
eyebrow: Espace 03 — Prototyper
intro: Un atelier équipé pour découper, imprimer, graver, souder. Pensé pour les bricoleur·euse·s et les artistes qui veulent passer du croquis à l'objet — vite, et bien.
equipment:
  - name: xTool M1
    type: Découpe & gravure
    specs:
      - Laser diode 10 W
      - Lame de découpe intégrée
      - Surface de travail 385 × 300 mm
      - Bois fin, acrylique, cuir, papier, vinyle
  - name: Epson SureColor SC-T3100N
    type: Traceur grand format
    specs:
      - Largeur jusqu'à 24" (A1)
      - Encres pigmentées UltraChrome XD2
      - Plans, posters, dessins haute définition
      - Connexion réseau, impression directe
  - name: Atelier électronique
    type: Prototypage & soudure
    specs:
      - Stations de soudure température réglable
      - Multimètres, alimentations stabilisées
      - Microcontrôleurs (Arduino, ESP32, RP2040)
      - Composants de base, fer, étain sans plomb
---

## Une boîte à outils partagée

Un fablab, ce n'est pas un magasin de machines : c'est un endroit où l'on apprend à les utiliser ensemble. La règle est simple — on apprend, on transmet, on documente. Une machine bien utilisée par dix personnes vaut mieux que dix machines mal utilisées chacune dans son coin.

## Équipements

Voir les fiches ci-dessous. Une initiation rapide est nécessaire avant la première utilisation autonome — c'est court, c'est gratuit, et ça évite les bobos.

## Ce qu'on y fait

- Prototypes d'objets, de mobilier léger, d'éditions imprimées.
- Pièces de découpe laser pour la scénographie de la micro-galerie.
- Tirages grand format pour l'atelier d'art, les expositions, les affiches.
- Bidouilles électroniques : capteurs, petits sons, lumières.

## Sécurité

Le laser et le fer à souder ne sont pas méchants — mais ils ne pardonnent pas l'inattention. Lunettes, ventilation, présence d'un·e référent·e formé·e : c'est non négociable.
